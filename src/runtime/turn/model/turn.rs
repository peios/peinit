...
pub enum RuntimeShutdownEventTurn {
    ...
    /// Not a source's turn: the coalesced reload owed after the boot window
    /// closed, run at the turn boundary that observed it close.
...
/// The one reload that follows the boot window, applying the definitions
/// the reloads during it left pending on not-yet-launched boot-plan members
/// (PEI-350).
#[derive(Debug, Clone, PartialEq, Eq)]
