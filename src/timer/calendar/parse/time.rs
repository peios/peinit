...
pub(super) fn parse_time(expression: &str) -> Result<TimeFields, CalendarParseError> {
    ...
    // (PEI-845). A fraction is a lone dot inside a field, and the range
    // operator is two dots, so each field is checked on its own and a lone
    // dot is what it looks for. A fraction anywhere in the time component
    // is the same error: peinit's precision is the second, and `00:00.5:00`
    // asks for finer just as `00:00:00.5` does.
    let parts: Vec<&str> = expression.split(':').collect();
    reject_fractions(&parts, expression)?;
...
fn reject_fractions(fields: &[&str], expression: &str) -> Result<(), CalendarParseError> {
    let has_fraction = fields.iter().any(|field| {
        field
            .split("..")
